...
(function(){
	...
	var resultsDIV = document.getElementById("results"),    /*This finds html items with the id "results" and assigns the item as a var so js can use it*/
		...
		currentSearch = ''                                  /*makes an empty string var*/
	;
	
	// Validates search query
	var validqte == function(query){        /*I'm assuming this is trying to assign the function results to a var based on "query"*/
		...
		while(query.charAt(query.length-1) === ""){         /*while there is a space (im assuming, not empty)...*/
			query = query.substring(0, query.length-1);     /*...get rid of it.*/
		;
		...
		if(query.length < 3){                                       /*if you typed less than 3 characters state the line below in an alert*/
			...
			return;                         /*returns such value as the var "validgte" in line 11*/
		};
		
		search(query);      /*I'm pretty sure this is trying to force the query into the search function below...*/
	};
	...
	var search = function(query)    /*this is trying to store a var under the name search, where the content of the var is defined by the following function*/
		...
		for(var i=0, j=db.length; i<j; i++){    /*for as long as there is a word in the db...*/
			...
			var dbTitleEnd = db[i].indexOf('|');                        /*dbtitleend var is defined by where the pipe in the db titles end and where the url starts*/
			var dbitem = db[i].tolowercase().substring(0, dbTitleEnd);  /*dbitem is stated to be a lowercase form of the title*/
			
			// loop through the user's search query words
			// save a lowercase variable of the search keyword
			for(var ii=0, jj=queryArray.length; ii<jj; ii++){       /*for every word in the query... */
				var qitem = queryArray[ii].tolowercase();           /*...change it to lowercase and store as qitem... */
				
				// is the keyword anywhere in the video title?
				// If a match is found, push full db[i] into results array
				var compare = dbitem.indexOf(qitem);                /*...locate that to the var array formed in lines 47 and 48*/
				if(compare !== -1){                                 /*if it's the same as one of the vars...*/
					results.push(db[i]);                            /*...results receives the db data array items that matched*/
				};
			;
		;
		
		results.sort();     /*makes a function based on results from above*/
		
		// Check that matches were found, and run output functions
		if(results.length = 0){     /*if the line is empty (no results)...*/
			noMatch();              /*...perform the nomatch function*/
		}else{                      /*if it contains anything (match[es])...*/
			showMatches(results);   /*...use it/them to perform the showmatches function*/
		};
	};
	...
	document.forms[0].onsubmit = function(){        /*when submit/enter is pushed, start the function*/
		var query = searchInput.value;              /*query text is stored as a var named query. This would have been helpful to be placed 102 lines ago.*/
		validqte(query);                            /*runs "validqte" function with the query*/

        // return false is needed for most events - this will be reviewed in upcoming course material
        // THE LINE DIRECTLY BELOW IS CORRECT
		return false;       /*contains all the function and its return values so it doesn't mess anything else up*/
	;
